...
\end{figure}
Graficamente osserviamo che $T_a([0,1])=[0,\frac a2]$, quindi $[0,\frac a2]$ \`e positivamente invariante, ma \`e invariante\footnote{ricordiamo che $X$ \`e positivamente invariante per $T$ se $T(X)\subseteq X$ mentre \`e invariante se $T(X)=X$} solo se $a\geq 1$.
\begin{figure}[!htb]
    \centering

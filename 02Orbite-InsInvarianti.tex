...
\dot y=g(\ulx,y)
\end{cases}\]
dove $(\ulf, g)(\ulx, y)=\Dc h_{h\ii(\ulx, y)}\circ F(h\ii(\ulx, y))$. Notiamo inoltre che\footnote{Per capire meglio i seguenti passaggi consigliamo di seguire corsi di Geometria differenziale. L'idea comunque dovrebbe essere intuitiva anche pensando a fatti di Analisi 2.}
\[\Dc h_{h\ii(\ulx,0)}\res{T_{h\ii(\ulx,0)}I_c}=\Dc(h\res{I_c})_{h\ii(\ulx,0)}:T_{h\ii(\ulx,0)}I_c\to T_{(\ulx,0)}\cpa{y=0}=\cpa{y=0},\] 
quindi $g(\ulx,0)=0$, in quanto $\dot I\res{I_c}=0, \nabla I\res{I_c}\neq 0\implies \imm F\res{I_c}\subseteq \nabla I^\perp =TI_c$\footnote{Stiamo sottointendendo la valutazione in un punto generico di $I_c$.}.\\
Osserviamo che se una soluzione rispetta il sistema
\[\begin{cases}
\dot \ulx=\ulf(\ulx,y)\\
\dot y=0\\
(\ulx,y)(0)=0
\end{cases}\]  
allora rispetta anche il sistema originale perch\'e non esce mai da $\cpa{y=0}$ e quindi la condizione $\dot y=0$ resta sempre vera. Per Cauchy-Lipschitz questa soluzione \`e l'unica che stavamo cercando e applicando $h\ii$ troviamo l'unica soluzione al sistema originale e, per quanto detto, questa rimane entro $I_c$.
\end{proof}
